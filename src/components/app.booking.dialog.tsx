...
export default function BookingDialog(props: Props) {
    const {isOpen, setIsOpen, movieSchedule, cinema, totalPrice, bookingSeats, setSelectMoviesSchedule} = props;
        ...
        try {
            ...
            const res = await fetch("http://127.0.0.1:8000/app/api/main/screen/seat/booking/", {
                ...
                body: JSON.stringify({
                    ...
                    showtime_id: movieSchedule.showtime_id,
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
            <div className="relative w-full max-w-xl bg-white dark:bg-gray-800 rounded-lg shadow-lg overflow-hidden">
                ...
                <div className="px-6 py-5 border-b rounded-t dark:border-gray-600 border-gray-300">
                    <h2 className="font-semibold pb-3 border-b border-dotted border-gray-300">{movieSchedule.movie_title}</h2>
                    <ul className="grid grid-cols-1 gap-y-4 py-4">
                        ...
                        <li className="flex items-center justify-between space-x-2 w-full pb-4 border-b border-dotted border-gray-300">
                            <div className="space-y-1 w-full">
                                <p className="text-gray-400 text-sm">PHÒNG CHIẾU</p>
                                <p className="font-semibold">{movieSchedule.screen_name}</p>
                            </div>
                            <div className="space-y-1 w-full">
                                <p className="text-gray-400 text-sm">ĐỊNH DẠNG</p>
                                <p className="font-semibold">{movieSchedule.screen_type}</p>
                            </div>
                        </li>
                        <li className="flex items-center justify-between space-x-2 w-full">
                            <div className="space-y-1 w-full">
